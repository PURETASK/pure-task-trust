...
export function MobileBottomNav() {
  ...
  const unreadMessages = threadsQuery.data?.reduce((sum, t) => sum + (t.unreadCount || 0), 0) ?? 0;
  ...
  const isActive = (item: NavItem) => {
    if (item.path === "/" || item.path === "/dashboard" || item.path === "/cleaner/dashboard" || item.path === "/admin/hub") {
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-background/95 backdrop-blur-lg border-t border-border pb-safe">
      <div className="flex items-stretch justify-around h-16 px-1">
        {config.items.map((item) => {
          ...
          const badge = isMessages ? unreadMessages : isAlerts ? notifCount : 0;
          ...
          return (
            <Link
              ...
              to={item.path}
              className={cn(
                "relative flex flex-col items-center justify-center flex-1 gap-0.5 transition-all duration-200",
                "active:scale-95"
              )}
            >
              {/* Active indicator bar */}
              {active && (
                <span className={cn("absolute top-0 left-1/2 -translate-x-1/2 h-0.5 w-8 rounded-b-full", config.activeIndicator)} />
              )}
              ...
              <div className={cn(
                ...
                <item.icon className={cn(
                  ...
                  active ? cn(config.accent, "stroke-[2.5]") : "text-muted-foreground stroke-[1.5]"
                )} />
                {badge > 0 && (
