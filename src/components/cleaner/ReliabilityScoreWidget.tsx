  ...
  return (
    ...
    >
      <div className="flex items-center justify-between text-sm">
        ...
        <span className={`font-semibold text-xs ${colorClass}`}>{displayValue}</span>
      </div>
...
export function ReliabilityScoreWidget() {
  ...
  // ── Real-time subscription: refresh whenever the score row updates ──────
  useEffect(() => {
  ...
  const metrics5 = [
    {
      ...
      displayValue: `${Math.round(scoreBreakdown.attendance)}%`,
      colorClass: scoreBreakdown.attendance >= 80 ? "text-success" : scoreBreakdown.attendance >= 60 ? "text-warning" : "text-destructive",
    },
    {
      icon: Clock,
      ...
      displayValue: `${Math.round(scoreBreakdown.punctuality)}%`,
      colorClass: scoreBreakdown.punctuality >= 80 ? "text-success" : scoreBreakdown.punctuality >= 60 ? "text-warning" : "text-destructive",
    },
    {
      icon: Camera,
      ...
      displayValue: `${Math.round(scoreBreakdown.photoCompliance)}%`,
      colorClass: scoreBreakdown.photoCompliance >= 80 ? "text-success" : scoreBreakdown.photoCompliance >= 60 ? "text-warning" : "text-destructive",
    },
    {
      icon: Star,
      label: "Client Rating",
      weight: 15,
      value: avgRating * 20,
      displayValue: avgRating > 0 ? `${avgRating.toFixed(1)} ★` : "No reviews",
      colorClass: avgRating >= 4 ? "text-success" : avgRating >= 3 ? "text-warning" : "text-destructive",
    },
    {
      icon: XCircle,
      ...
      weight: 5,
      value: noCancelPct,
      displayValue: `${Math.round(noCancelPct)}%`,
      colorClass: noCancelPct >= 80 ? "text-success" : noCancelPct >= 60 ? "text-warning" : "text-destructive",
    },
  ];

  // Generate a smart tip based on the weakest metric
  ...
  return (
    <Card className="border-border/60 overflow-hidden">
      ...
      <div className={`bg-gradient-to-r ${tierStyle.gradient} p-4 text-white`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-2xl">{tierStyle.icon}</span>
            <div>
              <p className="text-xs font-medium text-white/70 uppercase tracking-wider">Reliability Score</p>
              <div className="flex items-center gap-2">
          ...
          <div className="text-right">
            <Badge className="bg-white/20 text-white border-white/30 capitalize font-semibold mb-1">
              {tier} Tier
            </Badge>
            {tierStyle.next && pointsToNext > 0 && (
              <p className="text-xs text-white/70">
                <Zap className="h-3 w-3 inline mr-0.5" />{pointsToNext} pts to {tierStyle.next}
              </p>
            )}
            {tier === "platinum" && (
              <p className="text-xs text-white/70">🏆 Elite status!</p>
            )}
          </div>
        </div>
        ...
        {tierStyle.next && (
          <div className="mt-3">
            <div className="flex justify-between text-xs text-white/60 mb-1">
              <span>
                {tier === "bronze" ? 0 : tier === "silver" ? 50 : tier === "gold" ? 70 : 90}
              </span>
            ...
            <div className="h-2 rounded-full bg-white/20 overflow-hidden">
              <motion.div
                ...
                initial={{ width: 0 }}
                animate={{
                  width: `${Math.min(100, ((currentScore - (tier === "bronze" ? 0 : tier === "silver" ? 50 : tier === "gold" ? 70 : 90)) / (tierStyle.nextMin - (tier === "bronze" ? 0 : tier === "silver" ? 50 : tier === "gold" ? 70 : 90))) * 100)}%`
                }}
      ...
      <CardContent className="p-5 space-y-4">
        {/* 5 Metrics */}
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">Score Breakdown</p>
        ...
        <Button variant="outline" size="sm" asChild className="w-full rounded-xl gap-1.5">
