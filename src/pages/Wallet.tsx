...
import { Wallet as WalletIcon, Plus, ArrowUpRight, ArrowDownLeft, Clock, RefreshCw, Search, X, Zap, ChevronDown, ChevronUp, Shield, TrendingUp, CreditCard, FileText } from "lucide-react";
  ...
  return (
    <main className="flex-1 py-5 sm:py-10">
      ...
      <div className="container px-3 sm:px-4 lg:px-6 max-w-4xl">
        ...
        {/* ── HERO BALANCE ─────────────────────────────────────────────── */}
        <motion.div {...f(0.06)}>
          <div className="palette-card palette-card-blue mb-6 relative overflow-hidden"
            style={{
            ...
            <div className="relative p-6 sm:p-8">
              <div className="flex items-center gap-2 mb-3">
                <div className="palette-icon palette-icon-blue h-9 w-9">
                  <WalletIcon className="h-4 w-4" />
                </div>
                <span className="palette-label-blue font-black text-sm uppercase tracking-widest">Available Balance</span>
              </div>
                ...
                <div>
                  {isLoadingAccount ? <Skeleton className="h-20 w-56 rounded-2xl" /> : (
                    <div className="flex items-end gap-1 leading-none">
                      <span className="text-6xl sm:text-7xl font-black palette-label-blue tracking-tight">${availableCredits.toFixed(0)}</span>
                      <span className="text-2xl font-bold palette-label-blue opacity-50 mb-2">.{(availableCredits % 1).toFixed(2).slice(2)}</span>
                    </div>
                  )}
                </div>

                <div className="grid grid-cols-2 gap-3 lg:min-w-[220px]">
                  <div className="palette-card palette-card-amber p-0">
                    <div className="flex items-center gap-3 px-4 py-3">
                      <Clock className="h-4 w-4 palette-label-amber shrink-0" />
                      <div>
                        <p className="text-xs text-muted-foreground">Escrow</p>
                        <p className="font-black text-lg leading-none palette-label-amber">${heldCredits}</p>
                      </div>
                    </div>
                  </div>
                  <div className="palette-card palette-card-green p-0">
                    <div className="flex items-center gap-3 px-4 py-3">
                      <TrendingUp className="h-4 w-4 palette-label-green shrink-0" />
                      <div>
                        <p className="text-xs text-muted-foreground">Total</p>
                        <p className="font-black text-lg leading-none palette-label-green">${totalCredits}</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {/* Buy Credits CTA */}
        <motion.div {...f(0.1)}>
          <div className="palette-card palette-card-green p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-4">
              <div className="palette-icon palette-icon-green h-12 w-12">
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          ...
          <div className="palette-pill palette-pill-blue">
            <RefreshCw className="h-3.5 w-3.5" /> Instant Refunds
          </div>
          <div className="palette-pill palette-pill-amber">
            <Zap className="h-3.5 w-3.5" /> Auto Top-Up
          </div>
        </div>

        {/* Auto Top-Up */}
        <motion.div {...f(0.14)}>
          <div className="palette-card palette-card-purple mb-6 p-5">
            <button className="w-full flex items-center justify-between" onClick={() => setShowAutoTopUp(v => !v)}>
              ...
              <div className="flex items-center gap-2">
                {autoTopUpEnabled && <Badge className="palette-pill-green text-xs font-bold">Active</Badge>}
                    ...
                    {autoTopUpEnabled && (
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <div className="relative"><Input type="number" value={topUpThreshold} onChange={e => setTopUpThreshold(e.target.value)} className="palette-input palette-input-purple pr-16 h-9 text-sm" min="5" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">credits</span></div>
                        </div>
                        <div>
                          <Label className="text-xs text-muted-foreground mb-1.5 block">Amount to add</Label>
                          <div className="relative"><Input type="number" value={topUpAmount} onChange={e => setTopUpAmount(e.target.value)} className="palette-input palette-input-purple pr-16 h-9 text-sm" min="10" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">credits</span></div>
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <div className="palette-card palette-card-blue overflow-hidden">
            <div className="p-5 sm:p-6 pb-0">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge className="palette-pill-blue text-xs font-bold">{filteredLedger.length} entries</Badge>
              </div>
              <div className="flex flex-col sm:flex-row gap-2 pb-4">
                <div className="relative flex-1">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
                  <Input placeholder="Search transactions..." value={txSearch} onChange={e => setTxSearch(e.target.value)} className="palette-input palette-input-blue pl-9 pr-8 h-9 text-sm" />
                ...
                <div className="space-y-1">
                  {filteredLedger.map((entry, i) => {
                    ...
                    return (
                      ...
                      >
                        <div className={cn("h-11 w-11 rounded-xl border-2 flex items-center justify-center flex-shrink-0", isPositive ? "palette-icon-green" : "bg-muted border-border/40")}>
                          {isPositive ? <ArrowDownLeft className="h-5 w-5" /> : <ArrowUpRight className="h-5 w-5 text-muted-foreground" />}
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <p className={cn("font-black tabular-nums", isPositive ? "palette-label-green" : "text-foreground")}>
