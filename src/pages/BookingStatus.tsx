...
function getTimelineStep(statusKey: string): number {
  ...
  return steps.indexOf(statusKey);
}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
          ...
          <div className="text-center pt-4 pb-2">
            <motion.div
              ...
              className={`h-20 w-20 rounded-3xl ${config.bg} flex items-center justify-center mx-auto mb-4`}
            >
                    ...
                    return (
                      <div key={step.status} className="flex items-center flex-1 last:flex-none">
                        <div className="flex flex-col items-center gap-1">
                          ...
                          <span className={`text-[10px] font-medium whitespace-nowrap ${isCompleted ? "text-primary" : "text-muted-foreground"}`}>
                            {step.label}
                          </span>
                        </div>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                Booking Details
                <Badge variant={statusKey === "declined" ? "destructive" : statusKey === "active" ? "default" : "secondary"}>
                  {config.label}
                </Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-3 pb-4 border-b border-border">
                <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center font-bold text-primary text-lg flex-shrink-0">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-sm">{addressLine}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-border flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm capitalize">{(job.cleaning_type || "").replace("_", " ")} Clean</span>
                </div>
                <div className="text-right">
                  <p className="text-xs text-muted-foreground">Credits in escrow</p>
                  <p className="font-bold text-primary">{job.escrow_credits_reserved || 0}</p>
                </div>
              </div>
          ...
          <div className="space-y-2.5">
            {statusKey === "pending" && (
              <>
                ...
                {canCancel && (
                  <Button variant="outline" className="w-full gap-2 text-destructive border-destructive/30 hover:bg-destructive/5 rounded-xl" onClick={() => setShowCancelModal(true)}>
