    ...
    <>
      ...
      <div className="container py-8 space-y-6">
        <div>
          ...
          <p className="text-ink-muted text-sm mt-1">Side-by-side comparison to find your best match</p>
        </div>
                ...
                <CardContent className="p-5 space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-ink-muted flex items-center gap-1"><Star className="h-3.5 w-3.5" /> Rating</span>
                      <span className={`font-bold ${cleaner.id === getBest('avg_rating') ? 'text-success' : ''}`}>
                        {cleaner.avg_rating?.toFixed(1) || 'N/A'}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-ink-muted flex items-center gap-1"><DollarSign className="h-3.5 w-3.5" /> Rate</span>
                      <span className="font-bold">${cleaner.hourly_rate_credits}/hr</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-ink-muted flex items-center gap-1"><Shield className="h-3.5 w-3.5" /> Reliability</span>
                      <span className={`font-bold ${cleaner.id === getBest('reliability_score') ? 'text-success' : ''}`}>
                        {cleaner.reliability_score}%
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-ink-muted flex items-center gap-1"><Award className="h-3.5 w-3.5" /> Jobs Done</span>
                  ...
                  {cleaner.bio && <p className="text-xs text-ink-muted line-clamp-3">{cleaner.bio}</p>}
