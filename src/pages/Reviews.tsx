      ...
      <section className="relative py-10 sm:py-20 overflow-hidden">
        <div className="container relative px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-base sm:text-xl text-ink-muted mb-6 sm:mb-10 px-2">Every review is from a verified customer who experienced PureTask firsthand</p>
                  ...
                  <div>
                    <div className="flex gap-0.5 mb-1">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className={`h-5 w-5 sm:h-7 sm:w-7 ${i < Math.round(stats.averageRating) ? "fill-warning text-warning" : "text-ink-muted/30"}`} />
                      ))}
                    </div>
                    <p className="text-xs sm:text-sm text-ink-muted">{stats.totalReviews.toLocaleString()} verified reviews</p>
                  </div>
                </div>
                <div className="flex gap-3 sm:gap-6 pt-3 border-t border-border/50 w-full justify-center">
                  {[{ icon: TrendingUp, label: "98%", desc: "Would recommend" }, { icon: Award, label: "4.9★", desc: "Avg rating" }, { icon: Sparkles, label: "99%", desc: "Satisfaction" }].map((s, i) => (
                    <div key={i} className="text-center">
                      ...
                      <p className="text-[10px] sm:text-xs text-ink-muted leading-tight">{s.desc}</p>
                    </div>
                  ))}
                </div>
              </motion.div>
      ...
      <section className="relative py-8 sm:py-16 overflow-hidden">
        <div className="container relative px-4">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-6 sm:mb-8 pb-4 sm:pb-6 border-b border-border/50">
            <span className="text-xs sm:text-sm font-semibold text-ink-muted flex items-center gap-1.5 w-full sm:w-auto"><Filter className="h-4 w-4" />Filter:</span>
          ...
          {reviewsLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 sm:py-24 px-4 sm:px-6 text-center">
              <div className="h-16 w-16 sm:h-20 sm:w-20 rounded-full bg-warning/10 border border-hairline-soft flex items-center justify-center mx-auto mb-4 sm:mb-6">
              ...
              <p className="text-sm sm:text-base text-ink-muted max-w-md mb-6 sm:mb-8 leading-relaxed">
                {filterRating
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {reviews.map((review, index) => (
                <motion.article key={review.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }}>
                  <Card className="h-full hover:shadow-elevated transition-all duration-300 border-hairline-soft group overflow-hidden">
                    <CardContent className="p-4 sm:p-6">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex gap-0.5">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <Star key={i} className={`h-4 w-4 ${i < review.rating ? "fill-warning text-warning" : "text-ink-muted/20"}`} />
                          ))}
                        </div>
                      ...
                      <footer className="flex items-center gap-3 pt-4 border-t border-hairline-soft">
                        ...
                        <div>
                          ...
                          <time className="text-xs text-ink-muted" dateTime={review.created_at}>{formatDistanceToNow(new Date(review.created_at), { addSuffix: true })}</time>
                        </div>
                      </footer>
                    </CardContent>
                  </Card>
