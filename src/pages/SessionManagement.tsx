  ...
  return (
    <main className="flex-1 bg-background min-h-screen">
      ...
      <div className="container px-4 sm:px-6 py-5 sm:py-8 max-w-3xl">
        ...
        <motion.div {...f(0)} className="mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-ink-muted text-sm">Manage sessions, MFA, and login activity</p>
            </div>
          </div>
        </motion.div>
        ...
        {/* Sessions */}
        <motion.div {...f(0.08)}>
          <div className="rounded-3xl border-2 border-hairline-soft overflow-hidden">
            <div className="p-5 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-xl bg-muted border-2 border-hairline-soft flex items-center justify-center">
                    <Key className="h-5 w-5 text-ink-muted" />
                  </div>
                  <div>
                    <h2 className="text-lg font-poppins font-bold">Active Sessions</h2>
                    <p className="text-xs text-ink-muted">Devices currently logged in</p>
                  </div>
                </div>
                {sessions.length > 1 && (
                  <Button
                  ...
                  {sessions.map((session, i) => {
                    ...
                    return (
                      <motion.div key={session.id} {...f(0.1 + i * 0.03)}>
                        <div className="flex items-center gap-4 p-4 rounded-2xl border-2 border-hairline-soft bg-card">
                          <div className="h-11 w-11 rounded-xl bg-muted border-2 border-hairline-soft flex items-center justify-center flex-shrink-0">
                            <DeviceIcon className="h-5 w-5 text-ink-muted" />
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-ink-muted truncate">{session.browser || 'Unknown browser'}</p>
                            {session.last_active_at && (
                              <p className="text-xs text-ink-muted">Last active: {format(new Date(session.last_active_at), "MMM d, h:mm a")}</p>
                            )}
                          </div>
                          {!session.is_current && (
                            <Button
