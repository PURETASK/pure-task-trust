...
BEGIN
  ...
  FOR v_job IN
    ...
    WHERE j.status = 'completed'
      ...
      AND NOT EXISTS (
        SELECT 1 FROM disputes d WHERE d.job_id = j.id AND d.status IN ('open','investigating')
      )
      AND NOT EXISTS (SELECT 1 FROM cleaner_earnings ce WHERE ce.job_id = j.id)
  LOOP
    BEGIN
...
SELECT public.auto_release_expired_jobs(24);